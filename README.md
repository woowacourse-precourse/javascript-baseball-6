...
-   시작 메시지 출력 (o)
-   자동으로 컴퓨터의 수 3자리 생성 (o)
-   숫자 입력 메시지 출력 (o)
    // 문제점 : 입력을 받은 값을 활용하는 방법을 잘 모르는 중
-   사용자의 입력과 컴퓨터의 숫자 비교하는 함수 생성 :
    숫자의 위치와 값이 모두 같다면 스트라이크를, 숫자의 위치는 다르지만 값이 같다면 볼을, 아예 틀리면 낫임을 출력
-   해당 함수를 3스트라이크가 나올 떄 까지 계속해서 입력받는 값마다 실행시킴
-   만약 3스트라이크라면 게임 종료 메시지 출력
-   게임을 다시 시작 할 건지 묻는 메시지 출력 : 입력값이 1이면 게임을 다시 시작 , 2이면 게임을 끝냄
