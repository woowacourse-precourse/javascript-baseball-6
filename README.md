...
[x] 1. 랜덤한 정답 숫자 만들어내기 (`Random.pickNumberInRange() 사용`)
[x] 2. 플레이어가 입력한 값 유효성 검증
2-1. 숫자가 아닌 값이 포함된 경우
2-2. 숫자의 개수가 모자랄 때
2-3. 중복되는 숫자가 포함된 경우
2-4. 플레이어가 입력한 값에 오류가 없는 경우 정상적으로 값을 입력 받음
[x] 3. 사용자가 입력한 값이 정답과 같은지 확인
3-1. 정답이 아닌 경우 (힌트 출력)
3-2. 정답인 경우 -> 4로. 4. 재시작 여부 묻기
4-1. 1 누를 경우 숫자 초기화 되고 게임 재시작
4-2. 2 누를 경우 게임 종료
