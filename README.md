...
1. 숫자가 겹치지 않았는가?
2. 중간에 띄어쓰기가 없어야 한다.
3. 숫자를 정확하게 3개를 입력하였는가
4. 0이 없는가
  => 사용자가 잘못된 값을 입력한 경우 `throw`문을 사용해 예외를 발생시킨후 애플리케이션은 종료되어야 한다.

 - 볼, 스트라이크, 낫싱 (반복문)
1. 입력받은 숫자들을 /100, /10, %10을 통해 숫자를 리스트에 담는다.
2. 리스트 순서대로 반복문을 통해 비교한다.(스트라이크의 갯수)
3. 리스트 순서에 상관없이 숫자들을 비교한다. (스트라이크+볼의 갯수)
4. 스트라이크 + 볼의 갯수가 0이라면 낫싱을 출력

- 사용자가 3 스트라이크를 달성한 경우
 - 출력("3개의 숫자를 모두 맞히셨습니다! 게임 종료")
      ("게임을 새로 시작하려면 1, 종료하려면 2를 입력하세요.")
 - 게임을 새로 시작하는 경우 사용자로부터 다시 입력을 받는다.
 - 종료하는 경우 ("프로그램이 종료되었습니다.")
