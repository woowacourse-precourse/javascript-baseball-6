...
## ✨ 느낀 점
- 입출력 라이브러리가 따로 구현되어있어서 해당 파일 보면서 코드부터 이해해야 했다.
- async/await와 promise를 처음 봐서 관련 공부부터 해야했다.
- try/catch는 사용해본 적이 있어서 throw 쪽은 금방 구현할 수 있었다.
- jest를 처음 사용해봤지만 테스트 코드 보는 것은 별로 어렵지 않았다.
