...
**[예외]: "[ERROR]"로 시작하는 에러 문구를 출력하고 throw로 예외를 발생시킨 뒤 애플리케이션 종료**
...
- [예외] 입력의 길이가 3이 아닌 경우
- [예외] 입력이 모두 숫자가 아닌 경우
- [예외] 입력한 각 숫자가 모두 다른 숫자로 이루어지지 않은 경우
- 정답을 입력할 때까지 3번, 4번 기능을 반복해서 실행한다.

### 4. 입력받은 수와 정답 비교

- 같은 수가 같은 자리에 있으면 스트라이크,
  <br>같은 수가 다른 자리에 있으면 볼,
  <br>같은 수가 전혀 없으면 낫싱
  <br>을 카운트해서 해당 추론의 결과를 출력한다.
- 3자리 수가 모두 일치하면 문구를 출력한다.

### 5. 게임 종료 이후

- 선택지 문구를 출력하고 사용자로부터 선택지를 입력받는다.
- 1을 입력한 경우 1번 기능으로 돌아가 다시 실행한다.
- 2를 입력한 경우 애플리케이션을 완전히 종료한다.
