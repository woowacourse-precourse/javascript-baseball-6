...
const InputView = {
  readGuessNumber: async () => {
    ...
    if (!Validator.isValidLength(guessNumber, 3)) {
      throw new ValidationError("[ERROR] 3자리의 숫자가 아닙니다. 게임을 종료합니다.");
    }

    if (!Validator.isValidRange(guessNumber, 1, 9)) {
      throw new ValidationError("[ERROR] 1~9 사이의 숫자가 아닙니다. 게임을 종료합니다.");
    }
  ...
  readRestartAnswer: async () => {
    ...
    if (!Validator.isExitOrRestart(answer, 1, 2)) {
      throw new ValidationError("[ERROR] 1 또는 2를 입력하지 않았습니다. 게임을 종료합니다.");
    }
