...
class App {
  ...
  verifyInputFormat(input){
    ...
    if(USER_INPUT===''){
      throw new Error('아무것도 입력하지 않았습니다. 서로 다른 세 자리 숫자를 입력해 주세요.');
    }
    if(isNaN(USER_INPUT)){
      throw new Error('문자를 입력하였습니다. 서로 다른 세 자리 숫자를 입력해 주세요.');
    }
    if(!USER_INPUT.trim()){
      throw new Error('공백없이 입력해야 합니다. 서로 다른 세 자리를 입력해 주세요.');
    }
    if(USER_INPUT.length != 3){
      throw new Error('길이가 세 자리가 아닙니다. 서로 다른 세 자리 숫자를 입력해 주세요.');
    }
    if(!this.areDigitsUnique(USER_INPUT)){
      throw new Error('자릿수가 달라야 합니다. 서로 다른 세 자리를 입력해 주세요.');
    }
  }
  ...
  generateRandomNumber(){
    ...
    //Console.print(RANDOM_NUMBER_SET); // 테스트용 주석
