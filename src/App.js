...
class App {
  ...
  async createUserArray() {
    let array = await this.inputUserArray();
    let isError = await this.isUserArrayError(array);
    if (isError) {
      throw new Error("[ERROR] 숫자는 서로 다른 3자리로 입력해야합니다.");
    }
  }

  async inputUserArray() {
    try {
  ...
  async isUserArrayError(inputNum) {
    const inputSet = new Set(inputNum.split(''));
    if (inputNum.toString().length !== 3 || inputSet.size !== 3) {
      return true;
    } else {
