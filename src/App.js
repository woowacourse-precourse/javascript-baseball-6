...
async function playGame() {
  let computer = computerNumberPick();
  ...
  while (true) {
    ...
    user_Number = user_Number.split('').map(Number); // 문자로 받은 userNumber를 배열로 바꾸기
    ...
    for (let count = 0; count < 3; count++) {
      if (user_Number[count] == computer[count]) {
        count_Strike++;
      }
      if (user_Number.includes(computer[count])) {
    ...
    if (count_Ball != 0) {
      print_Sentence += count_Ball + "볼";
    }

    if (count_Strike != 0) {
      print_Sentence += count_Strike + "스트라이크";
    }
  ...
  let start_Num_Setting = await Console.readLineAsync("게임을 새로 시작하려면 1, 종료하려면 2를 입력하세요.");
  if (start_Num_Setting == 1)(await playGame());
}
