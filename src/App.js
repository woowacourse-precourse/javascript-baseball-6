...
class App {
  ...
  validation(input, mode) {
    if (mode === this.IN_GAME) {
      const reg = /[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/ ]/gim;
      if (!Number(input)) throw new Error("[ERROR] 숫자가 잘못된 형식입니다.");
      else if (input.replace(reg, "").length !== 3) throw new Error("[ERROR] 숫자가 잘못된 형식입니다.");
      else if (input.includes(" ")) throw new Error("[ERROR] 공백은 포함할 수 없습니다.");
      else if (input.includes("0")) throw new Error("[ERROR] 1~9 사이의 숫자만 입력이 가능합니다.");
      else if (new Set(input).size !== 3) throw new Error("[ERROR] 중복된 숫자는 입력할 수 없습니다.");
    }
