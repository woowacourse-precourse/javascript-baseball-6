...
class App {
    ...
    while (true) {
      ...
      if (countS !== 0 || countB !== 0) {
        MissionUtils.Console.print(
          //prettier-ignore
          `${countB ? countB + "볼 " : ""}${countS ? countS + "스트라이크" : ""}`
        );
      } else {
      ...
      if (countS === 3) {
        MissionUtils.Console.print(
          `${countS}개의 숫자를 모두 맞히셨습니다! 게임 종료`
        );
        const regame = await MissionUtils.Console.readLineAsync(
  ...
  strikeCount = (userInput) =>
    [...userInput].map((v) => +v).filter((v, i) => v === this.computer[i])
      .length;
  ballCount = (userInput) =>
    6 - new Set([...[...userInput].map((v) => +v), ...this.computer]).size;
}
