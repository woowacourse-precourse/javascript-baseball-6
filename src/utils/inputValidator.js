...
export const validateUserInput = (value) => {
  if (!validateIsNum(value)) throw Error(INPUT_ERROR_MESSAGE.NUM_ERR);
  else if (validateIsLengthNotThree(String(value)))
    throw Error(INPUT_ERROR_MESSAGE.LENGTH_ERR);
  else if (validateIncludeZero(String(value)))
    throw Error(INPUT_ERROR_MESSAGE.INCLUDE_ZERO_ERR);
  else if (validateIsNumDuplicated(String(value)))
