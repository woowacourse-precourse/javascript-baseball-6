...
- [✅] 컴퓨터 랜덤 숫자 생성
  ...
  - [x] 게임시작시 컴퓨터가 1~9 범위의 중복되지 않는 랜덤한 숫자 세자리를 가지고 있어야한다.
  - [x] 게임재시작시 새로운 랜덤숫자를 만들어야한다.
  - [x] 게임종료되기 전까지 이 숫자는 고정되어야 한다.

- [✅] 입력 유효성 검사 (input 필수)

  - [x] 숫자만으로 구성되어 있는가? (영어, 한글, 특수문자, 공백 안됨)
  - [x] 중복되지 않는 수들로만 구성되어 있는가?
  - [x] 1~9범위 내의 숫자인가 (0을 포함하면 안됨)
  - [x] 재시작 입력시 1 또는 2 인 숫자인가?
  - [x] 입력 예외발생시 throw 에러메세지가 출력되고 앱이 종료되어야 한다.

- [✅] 볼, 스트라이크, 낫싱 계산

  - [x] 사용자숫자와 컴퓨터숫자를 비교한다.
  - 숫자와 자리의 위치까지 일치하면 스트라이크 + 1
  - 숫자만 일치하고 위치는 다르면 볼 + 1
  - 숫자가 모두 일치하지 않으면 낫싱 (볼 0, 스트라이크 0)
  - 숫자와 자리의 위치가 전부 일치하면 3스트라이크, (게임종료 조건)
  - [x] 볼과 스트라이크의 숫자를 계산해낸 결과값을 출력해야한다.

- [✅] 재시작과 종료

  - [x] 3자리 모두 일치시 게임종료 문구 출력과 재시작 입력을 받아야한다.
  - [x] 사용자가 1을 입력했을때, 컴퓨터가 랜덤한 숫자를 가지고 게임이 다시 시작되어야한다. (게임시작문구는 제외)
  - [x] 사용자가 2를 입력했을때, 애플리케이션이 종료되어야 한다.

- [✅] 출력 분기
  - 볼과 스트라이크 모두 0이면 `낫싱`
  - 볼만 있을 경우 `n볼`
  - 스트라이크 있을 경우 `n스트라이크`
  - 볼과 스트라이크 1개 이상씩 있을 경우 `n볼 n스트라이크` 가 출력되어야 한다.
