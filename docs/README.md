...
- 1. 각각 다른 무작위의 숫자 3개 생성
- 2. 유저가 정답을 제출할 수 있는 입력창 구현
- 3. 유저가 제출한 정답이 올바른 규칙인지 검증

  - 3.1 입력 값이 1~9 사이의 숫자가 아닐 경우 에러처리
  - 3.2 입력 값 숫자의 자릿수가 3자리가 아닐경우 에러처리
  - 3.3 입력 값 숫자의 중복일 경우 에러처리

- 4. 유저가 제출한 정답의 결과 생성
- 5. 유저가 입력한 정답의 결과 출력
- 6. 정답을 맞출 때까지 2~5번 기능의 반복 기능 구현
- 7. 유저가 정답을 맞출 경우 새로운 게임의 시작 혹은 종료를 위한 입력창 구현
  - 7.1 "1" 또는 "2"외의 입력값을 제출한 경우 예외처리
  - 7.2 "1"을 입력하는 경우 새로운 게임 시작 구현
  - 7.3 "2"를 입력하는 경우 게임 종료 구현
