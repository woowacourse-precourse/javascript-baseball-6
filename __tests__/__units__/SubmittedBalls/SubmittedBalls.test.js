...
describe('SubmittedBalls 테스트', () => {
  it.each([
    ...
    expect(submittedBalls.targetBalls).toEqual(TargetBalls.of(input));
  });

  it('`increaseStrike` 메서드는 `score`의 `strike`를 `1` 증가시킨다.', () => {
    const submittedBalls = SubmittedBalls.of([1, 2, 3]);
    submittedBalls.increaseStrike();
    expect(submittedBalls.score.strike).toBe(1);
    submittedBalls.increaseStrike();
    expect(submittedBalls.score.strike).toBe(2);
  });

  it('`increaseBall` 메서드는 `score`의 `ball`을 `1` 증가시킨다.', () => {
    const submittedBalls = SubmittedBalls.of([1, 2, 3]);
    submittedBalls.increaseBall();
    expect(submittedBalls.score.ball).toBe(1);
    submittedBalls.increaseBall();
    expect(submittedBalls.score.ball).toBe(2);
  });
});
